    ...
    def __init__(self, parent, topOne):
        ...
        def resetDefaults():
            ...
            if (a):
                ...
                config['settings'] = {'theme': self.themeChosen, 'bar': self.barColor, 'highlight': self.highlightColor, 'accent': self.activeColor, 'dir': "C:\\", 'tab': self.defaultTabVariable.get()}
                ...
                config['model'] = {'mc': self.defaultMCVariable.get(), 'fit': self.defaultFitVariable.get(), 'weight': self.defaultWeightingVariable.get(), 'alpha': self.defaultAlphaVariable.get(), 'ellipse': self.ellipseColor}
        ...
        def saveSettings():
            ...
            try:
                ...
                messagebox.showerror("Value error", "Error 44:\nPlease enter a positive integer for number of comment lines")
                return
            #try:
            config = configparser.ConfigParser()
            config['settings'] = {'theme': self.themeChosen, 'bar': self.barColor, 'highlight': self.highlightColor, 'accent': self.activeColor, 'dir': self.defaultDirectoryEntry.get(), 'tab': self.defaultTabVariable.get()}
            ...
            config['model'] = {'mc': numMCDefault, 'fit': self.defaultFitVariable.get(), 'weight': self.defaultWeightingVariable.get(), 'alpha': numAlphaDefault, 'ellipse': self.ellipseColor}
            config['error'] = {'detrend': self.defaultDetrendVariable.get(), 'alphaError': self.defaultAlphaCheckboxVariable.get(), 'betaError': self.defaultBetaCheckboxVariable.get(), 'reError': self.defaultReCheckboxVariable.get(), 'gammaError': self.defaultGammaCheckboxVariable.get(), 'deltaError': self.defaultDeltaCheckboxVariable.get(), 'errorWeighting': self.defaultErrorWeightingVariable.get(), 'errorMA': self.defaultMovingAverageVariable.get()}
            config['custom'] = {'askCustomExit': self.customFormulaExitAlertVariable.get()}
            if (os.path.exists(os.getenv('LOCALAPPDATA')+r"\MeasurementModel")):
                with open(os.getenv('LOCALAPPDATA')+r"\MeasurementModel\settings.ini", 'w+') as configfile:
                    config.write(configfile)
                    configfile.close()
            ...
            #except:
            #    messagebox.showerror("File error", "Error 45:\nError in applying or saving settings")
